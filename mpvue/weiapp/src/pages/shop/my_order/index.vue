    ...
    <van-tabs :active="tabActive" @change="toggleTab" swipeable border="false" swipe-threshold="6">
      <van-tab title="全部">
        <div class="order-item" v-for="(order, index) in orders" :key="index">
          <a :href="'../order_detail/main?order_id='+order.id" class="order-item__hd" v-for="(goods, idx) in order.goods_datas" :key="goods.id">
          ...
          <div class="order-item__ft" v-if="order.pay_status == 0 && order.refund == 0 || order.status_code > 0 && order.status_code < 4 && order.refund == 0 || order.refund == 0 && order.status_code == 3|| order.refund == 0 && order.pay_status == 1 && order.status_code >3 && order.status_code != 7">
            ...
            <a :href="'../logistics/main?order_id=' + order.id" v-if="order.status_code > 0 && order.status_code < 4" class="u-button u-button--border">查看物流</a>
      ...
      <van-tab title="待支付">
        <div class="order-item" v-for="(Pay, index) in waitPay" :key="index">
          <a :href="'../order_detail/main?order_id='+Pay.id" class="order-item__hd" v-for="(goods, idx) in Pay.goods_datas" :key="goods.id">
      ...
      <van-tab title="待收货">
        <div class="order-item" v-for="(collect, index) in waitCollect" :key="index" v-if="waitCollect">
          <a :href="'../order_detail/main?order_id='+collect.id" class="order-item__hd" v-for="(goods, idx) in collect.goods_datas" :key="goods.id">
            <image class="u-goods__img" :src="goods.cover"></image>
          ...
          <div class="order-item__ft">
            ...
            <a :href="'../logistics/main?order_id=' + collect.id" v-if="collect.status_code > 0 && collect.status_code < 4" class="u-button u-button--border">查看物流</a>
      ...
      <van-tab title="待评价">
        <div class="order-item" v-for="(comment, index) in waitComment" :key="index">
          <a :href="'../order_detail/main?order_id='+comment.id" class="order-item__hd" v-for="(goods, idx) in comment.goods_datas" :key="goods.id">
            <image class="u-goods__img" :src="goods.cover"></image>
